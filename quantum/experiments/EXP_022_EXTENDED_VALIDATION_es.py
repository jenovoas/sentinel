...
"""
📊 EXP-022: Validación Estadística Extendida (1000+ Señales)
===========================================================

OBJETIVO: Validar S60 con análisis estadístico robusto
          - 1000+ señales con entropía real
          - Medir media/desvío de divergencias
          - Detectar edge cases
          - Validar para producción

CRITERIOS DE ÉXITO:
- Divergencia media < 0.01
...
# REVIEW: ruta absoluta reemplazada por relativa al proyecto
...
print("📊 EXP-022: VALIDACIÓN ESTADÍSTICA EXTENDIDA")
print("=" * 70)
...
print(f"   Threshold media: {THRESHOLD_MEAN}")
print(f"   Threshold desvío: {THRESHOLD_STD}")
...
if len(divergences_lyap) > 0:
    ...
    print(f"   Media:     {mean_div:.6f} {'✅' if mean_div < THRESHOLD_MEAN else '❌'} (threshold: {THRESHOLD_MEAN})")
    print(f"   Desvío:    {std_div:.6f} {'✅' if std_div < THRESHOLD_STD else '❌'} (threshold: {THRESHOLD_STD})")
    print(f"   Mín:       {min_div:.6f}")
    print(f"   Máx:       {max_div:.6f}")
    print(f"   Mediana:   {p50:.6f}")
    print(f"   P95:       {p95:.6f}")
    print(f"   P99:       {p99:.6f}")
    print(f"   < {THRESHOLD_99PCT}: {pct_under:.2f}% {'✅' if pct_under >= 99 else '❌'} (objetivo: 99%)")
    ...
    print(f"\n📊 Distribución de Valores:")
    print(f"   Float media: {mean_lyap_float:.4f}")
    print(f"   S60 media:   {mean_lyap_s60:.4f}")
    ...
    if edge_cases:
        ...
        for idx, div, f_val, s_val in edge_cases[:5]:  # Mostrar primeros 5
            print(f"   Señal #{idx}: Δ={div:.6f}, float={f_val:.4f}, s60={s_val:.4f}")
    else:
        print(f"\n✅ No hay edge cases detectados (todas Δ < 0.05)")
...
if all_pass:
    ...
    print("   - Divergencia media dentro del threshold")
    print("   - Divergencia desvío dentro del threshold")
    ...
    if mean_div >= THRESHOLD_MEAN:
        print(f"   ❌ Divergencia media alta: {mean_div:.6f}")
    if std_div >= THRESHOLD_STD:
        print(f"   ❌ Desvío divergence alto: {std_div:.6f}")
    if pct_under < 99:

...
# Sentinel imports
class SafeLfmClient:
    """Client for local LFM 2.5 on Vulkan GPU (OpenAI streaming format)."""
    def __init__(self, base_url="http://127.0.0.1:8080"):
        self.base_url = base_url.rstrip("/")
        self.client = httpx.AsyncClient(timeout=45.0)

    async def stream_generate(self, prompt: str, model: str = "lfm2.5-2.6b", system_prompt: str = "Eres Sentinel AI. Responde con precisión técnica y concisión."):
        payload = {
            "messages": [
        ...
        try:
            async with self.client.stream("POST", f"{self.base_url}/v1/chat/completions", json=payload) as response:
                if response.status_code != 200:
                    yield f"Error del servidor LFM ({response.status_code})"
                    return
            ...
            yield f"\n[Error de conexión con LFM 2.5 (:8080): {e}]"
...
try:
    ...
    class DummyShield:
        def sanitize_prompt(self, p): return p, True
    aiops_shield = DummyShield()
...
class SentinelTUI(App):
    ...
    def __init__(self):
        ...
        # Determine which AI provider to use (default: LFM 2.5 local on Vulkan GPU)
        ai_provider = os.getenv("SENTINEL_AI_PROVIDER", "lfm").lower()
        ...
        if ai_provider == "antigravity" and ANTIGRAVITY_AVAILABLE:
            ...
            self.ai_client = SafeLfmClient(base_url=os.getenv("LFM_BASE_URL", "http://127.0.0.1:8080"))
            self.ai_provider = "lfm"
            self.ai_model = "lfm2.5-2.6b"

...
/**
 * Create Sovereign Matrix (60 nodes hexagonal lattice)
 */
export function createSovereignMatrix(sequence: number[] = []): {
    nodes: THREE.Mesh[];
    filaments: THREE.Line[];
    massCenter: THREE.Mesh;
    group: THREE.Group;
} {
    const group = new THREE.Group();
    ...
    // 1. Hexagonal Lattice (60 nodes)
    // We create a spiral/hex pattern for 60 nodes
    ...
    for (let i = 0; i < 60; i++) {
        ...
        const radius = 5 + Math.sin(i * 0.5) * 0.5; // Slight hexagonal ripple
    ...
    // 2. Axion Flow Filaments (Salto-17 Sequence)
    if (sequence.length > 0) {
        const points: THREE.Vector3[] = [];
        sequence.forEach((nodeIdx) => {
            const node = nodes[nodeIdx];
            if (node) points.push(node.position.clone());
        });
    ...
    // 3. Inertial Mass Center (Golden Light)
    ...
    const massCenter = new THREE.Mesh(centerGeometry, centerMaterial);
    group.add(massCenter);

    return { nodes, filaments, massCenter, group };
}

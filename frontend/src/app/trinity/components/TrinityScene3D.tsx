...
import { createMerkabah, createHierarchy, createFlowerOfLife, createSovereignMatrix, detectGPUQuality } from '../utils/geometry';
...
export default function TrinityScene3D({
    ...
    const matrixRef = useRef<ReturnType<typeof createSovereignMatrix> | null>(null);
    ...
    useEffect(() => {
        ...
        const matrix = createSovereignMatrix(phaseJump?.sequence || []);
        matrix.group.position.set(0, 0, 0);
        ...
        const animate = () => {
            ...
            // Animate Sovereign Matrix
            if (matrixRef.current) {
                const { nodes, filaments, massCenter, group } = matrixRef.current;

                // Slow rotation of the entire matrix
                group.rotation.y = time * 0.1;

                // Animate nodes pulsing
                nodes.forEach((node, i) => {
                    const pulse = Math.sin(time * 3 + i * 0.2) * 0.02 + 1;
                    node.scale.set(pulse, pulse, pulse);

                    // Emissive intensity based on coherence
                    if (node.material instanceof THREE.MeshPhongMaterial) {
                        node.material.emissiveIntensity = 0.5 + Math.sin(time * 5 + i) * 0.3;
                    }
                });

                // Pulse mass center (Golden Light)
                const massPulse = 1 + Math.sin(time * 4) * 0.1 + (audioAmplitude * 0.5);
                massCenter.scale.set(massPulse, massPulse, massPulse);

                // Animate filaments (Axion Flow)

...
| Mejora | Esfuerzo | Riesgo | Impacto | Prioridad | Estado |
|--------|----------|--------|---------|-----------|--------|
| **Zero-init hardening** | 5 min | Ninguno | Crítico (previene leaks KASLR) | **P0 - Inmediato** | ✅ **COMPLETADO** |
| **Kani verification harness** | 15 min | Bajo | Alto (garantía formal bridge C→Rust) | **P1 - Esta semana** | ⏳ Pendiente |
| **Decision tree embebido (eBPF)** | 30 min | Medio | Alto (detección en kernel, sin userspace) | **P2 - Próximo sprint** | ⏳ Pendiente |
| **FFT + Q-factor detector** | 20 min | Medio | Alto (beaconing/DNS tunneling/C2) | **P2 - Próximo sprint** | ⏳ Pendiente |
...
### 3.1 P0: Zero-Init Hardening (5 min) ✅ **COMPLETADO**
...
**Archivos corregidos**:
- `ebpf/ai_guardian.c` → `send_cortex_event()` (línea 99)
- `ebpf/guardian_alpha_lsm.c` → `log_event()` (línea 70)
- `ebpf/float_detector.c` → `emit_event()` **YA LO HACÍA** (línea 67)
- `ebpf/guardian_gamma.c` → `emit_gamma()` (línea 98)

**Fix aplicado**: Añadir `__builtin_memset(e, 0, sizeof(*e))` antes de rellenar campos.
...
**Compilación verificada**: `./compilar_ebpf.sh` → ✅ todos los .o generados correctamente.
...
pub struct PeriodicDetector {
    buffer: VecDeque<S60>,
    capacity: usize,
    sample_rate: S60,  // eventos/segundo estimado
    q_threshold: S60,  // ej. S60[50;0]
}

impl PeriodicDetector {
    pub fn new(capacity: usize, sample_rate_hz: f64, q_threshold: S60) -> Self {
        Self {
            buffer: VecDeque::with_capacity(capacity),
            capacity,
            sample_rate: S60::from_raw((sample_rate_hz * S60::SCALE_0 as f64) as i64),
            q_threshold,
        }
    }
    
    pub fn push(&mut self, entropy_raw: u64) -> Option<Detection> {
        self.buffer.push_back(S60::from_raw(entropy_raw as i64));
        if self.buffer.len() > self.capacity {
            self.buffer.pop_front();
        }
        
        if self.buffer.len() == self.capacity && self.buffer.len() % 100 == 0 {
            let signal: Vec<S60> = self.buffer.iter().copied().collect();
            if let Ok(spectrum) = fft_s60(&signal) {
                if let Ok(q) = q_factor_s60(&spectrum, self.sample_rate) {
                    if q > self.q_threshold {
                        return Some(Detection { q_factor: q, timestamp: /* ... */ });
                    }
                }
            }
        }
...
**Integración**: En `ebpf_cortex_bridge.rs` `run_monitor()` → tras `resonant.push(entropy)`, también `periodic.push(entropy_raw)`.

---

## 4. Próximos Pasos Inmediatos

1. **P0** ✅ — Zero-init en `ai_guardian.c`, `guardian_alpha_lsm.c`, `float_detector.c`, `guardian_gamma.c` **COMPLETADO**.
2. **P1** — Crear `sentinel-cortex/kani/` y harness para `parse_event`.
3. **P2** — Definir features exactas del decision tree + entrenar modelo offline.
4. **P2** — Implementar `PeriodicDetector` e integrar en bridge.

---

## 5. Referencias

- **Heimdall**: Dasu et al., *Formally Verified Automated Migration of Legacy eBPF Programs to Rust*, 2026 (arXiv:2605.25411)
- **eBPF IDS/ML**: Bachl et al., *A flow-based IDS using Machine Learning in eBPF*, 2022 (arXiv:2102.09980)
- **eBPF DDoS IoT**: Tolay, *eBPF-Based Real-Time DDoS Mitigation for IoT Edge Devices*, 2025 (arXiv:2508.00851)
- **Time Crystals**: Keesling et al., *Gravitational Wave Memory in a Time Crystal*, 2025 (arXiv:2509.21959); Yao et al., *Spin-Induced Fractal Time-Crystal-Like Dynamics*, 2026 (arXiv:2606.30890)
- **YATRA Spec**: `sentinel/constraints/YATRA_SPEC.md` — Contrato aritmético inmutable (no floats en S60)
- **S60 Math**: `sentinel-cortex/src/math/s60.rs`, `s60_math.rs`, `harmonic_logic.rs`

---

*Documento vivo — actualizar tras cada mejora completada.*
...
The `z.string().datetime()` method enforces ISO 8601; default is no timezone offsets and arbitrary sub-second Precisión de Juguete.
...
The `z.string().time()` method validates strings in the format `HH:MM:SS[.s+]`. The second can include arbitrary Precisión de Juguete. It does not allow timezone offsets of any kind.
...
You can set the `precision` option to constrain the allowable Precisión de Juguete.

...
class BCIController:
    ...
    def _play_tone(self, freq, duration, volume=S60(0, 30, 0)):
    ...
    def trigger_qualia(self, event_type):
        ...
        if event_type == "KERNEL_BLOCK":
            ...
            self._play_tone(3500, S60(1, 0, 0), volume=0.7)

        elif event_type == "RESONANCE_EVENT":
            print("🌌 [BCI] Qualia: FIRST RESONANCE (S60(153, 24, 0) MHz Harmonic)")
            # Golden Ratio Harmonic Sequence
            # S60(153, 24, 0) is too high for audio, so we scale by Phi (1.618) harmonics in audible range
            base = S60(153, 24, 0) * 4  # ~613 Hz
            phi = 1.618
    ...
    def play_base60_pattern(self, residue):
        ...
        freq = 500 + (residue * 12)
        self._play_tone(freq, S60(0, 30, 0))

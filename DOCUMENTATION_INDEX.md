# 🗂️ SENTINEL CORTEX: MASTER DOCUMENTATION INDEX

Este documento sirve como el mapa maestro de todo el conocimiento y código funcional del proyecto Sentinel, cumpliendo con el **Axioma VI: Preservación Absoluta**.

---

## 🛡️ Núcleo y Directivas
- **[AI_PRIME_DIRECTIVES.md](file:///home/jnovoas/dev/sentinel/AI_PRIME_DIRECTIVES.md)** - Los Axiomas Inmutables (YATRA, Preservación, Bio-Centrismo).
- **[README.md](file:///home/jnovoas/dev/sentinel/README.md)** - Introducción general al sistema.
- **[ARCHITECTURE.md](file:///home/jnovoas/dev/sentinel/ARCHITECTURE.md)** - Arquitectura de alto nivel.

## 🧪 Sistemas Cuánticos y YATRA (Base-60)
- **Directorio: [/quantum](file:///home/jnovoas/dev/sentinel/quantum/)**
  - **[yatra_guard.py](file:///home/jnovoas/dev/sentinel/quantum/yatra_guard.py)** - Defensor activo contra fricción decimal.
- **[TesisResonancia.md](file:///home/jnovoas/dev/sentinel/TesisResonancia.md)** - Fundamentos teóricos de la resonancia cuántica.

## 📚 Documentación de Desarrollo y Patentes
- **Directorio: [/docs](file:///home/jnovoas/dev/sentinel/docs/)**
  - **[README.md](file:///home/jnovoas/dev/sentinel/docs/README.md)** - Índice de documentos activos y de referencia.
  - **[CORTEX_NARRATIVA_COMPLETA.md](file:///home/jnovoas/dev/sentinel/docs/CORTEX_NARRATIVA_COMPLETA.md)** - Narrativa maestra para inversores.
  - **[NEURAL_ARCHITECTURE.md](file:///home/jnovoas/dev/sentinel/docs/NEURAL_ARCHITECTURE.md)** - Documentación completa de patentes.

## 🛠️ Operación y Mantenimiento
- **[SERVICIOS_ACTIVOS.md](file:///home/jnovoas/dev/sentinel/SERVICIOS_ACTIVOS.md)** - Estado de servicios y orquestación.
- **[CHECKLIST.md](file:///home/jnovoas/dev/sentinel/CHECKLIST.md)** - Listas de control operativo.
- **[CHANGELOG.md](file:///home/jnovoas/dev/sentinel/CHANGELOG.md)** - Historial de evolución del sistema.

## 🔬 Observabilidad y Salud
- **[OBSERVABILITY.md](file:///home/jnovoas/dev/sentinel/OBSERVABILITY.md)** - Protocolos de monitoreo.
- **[OBSERVABILITY-STATUS.md](file:///home/jnovoas/dev/sentinel/OBSERVABILITY-STATUS.md)** - Estado actual de la observabilidad.

---

> **NOTA:** Cualquier nuevo documento significativo DEBE ser indexado aquí inmediatamente después de su creación.

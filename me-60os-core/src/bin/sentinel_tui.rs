...
impl AppState {
    fn new() -> Self {
        ...
        Self {
            ...
            chat_messages,
            security_events,
            energy_history: VecDeque::from(vec![45, 48, 52, 50, 55, 60, 58, 62, 65, 70, 68, 72, 75]),
...
fn query_lfm_25(prompt: &str, persona: &str) -> String {
    ...
    match client.post("http://127.0.0.1:8080/v1/chat/completions").json(&payload).send() {
        Ok(resp) => {
                                ...
                                thread::spawn(move || {
                                    ...
                                    if let Ok(mut s_ai) = state_ai.lock() {
                                        ...
                                        s_ai.security_events.push_back(SecurityEvent {
                                            tag: "TRUTHSYNC".into(),
                                            message: format!("Afirmación validada en <85μs (L={})", prompt.len()),
                                            level: "OK".into(),
                                        });
...
fn draw_header(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let title = Paragraph::new(Line::from(vec![
        ...
        Span::styled("SOVEREIGN TUI", Style::default().fg(WHITE).add_modifier(Modifier::DIM)),
    ]))
...
fn draw_chat_stream(f: &mut Frame, area: Rect, state: &AppState) {
    let block = Block::default()
        ...
        .title(Span::styled(" 💬 NEURAL STREAM (LFM 2.5) ", Style::default().fg(CYAN).add_modifier(Modifier::BOLD)));
...
fn draw_side_telemetry(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let lat_block = Block::default()
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded)
        .title(Span::styled(" 🌊 RESONANT LATTICE (/dev/shm) ", Style::default().fg(PURPLE).add_modifier(Modifier::BOLD)));
    ...
    let stats = Paragraph::new(Line::from(vec![
        ...
        Span::styled("[1; 00, 00] ", Style::default().fg(GREEN).add_modifier(Modifier::BOLD)),
    ...
    let sec_block = Block::default()
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded)
        .title(Span::styled(" 🛡️ TRUTHSYNC & AUDIT TRAIL ", Style::default().fg(GREEN).add_modifier(Modifier::BOLD)));
...
fn draw_input(f: &mut Frame, area: Rect, state: &AppState) {
    ...
    let input_block = Block::default()
        .borders(Borders::ALL)
        .border_type(BorderType::Rounded)
        .title(Span::styled(" PROMPT ", Style::default().fg(WHITE).add_modifier(Modifier::BOLD)));

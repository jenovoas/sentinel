...
fn main() {
    ...
    println!(
        "# target_sigma (raw) = {}",
        stabilizer.target_sigma.to_raw()
    );
    println!(
        "# damping_factor (raw) = {}",
        stabilizer.damping_factor.to_raw()
    );
    ...
    for step in 0..steps {
        ...
        // Guardrails
        if next_flux > stabilizer.limit_upper {
            next_flux = stabilizer.limit_upper;
        }
        if next_flux < stabilizer.limit_lower {
            next_flux = stabilizer.limit_lower;
        }
        ...
        println!(
            "{},{},{},{}",
            step,
            stabilizer.current_flux.to_raw(),
            noise_scaled.to_raw(),
            drift_raw
        );
    }
    ...
    eprintln!(
        "Drift final (raw): {} (|{:.2}%|)",
        final_drift,
        (final_drift as f64 / target_raw as f64) * 100.0
    );
    eprintln!("Drift máximo (raw): {}", max_drift);
    eprintln!("Drift promedio (raw): {}", avg_drift);
    eprintln!(
        "Flux dentro de guardrails: {}",
        stabilizer
            .history
            .iter()
            .all(|f| *f >= stabilizer.limit_lower && *f <= stabilizer.limit_upper)
    );

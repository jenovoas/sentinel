...
fn crystal_latency(phase: &str, ticks: u64, pid: &mut Option<S60PID>, lattice: &mut Option<ResonantMatrix>) {
...
fn crystal_drift(phase: &str, ticks: u64, pid: &mut Option<S60PID>, lattice: &mut Option<ResonantMatrix>) {
    let mut clock = IsochronousClock::new_internal();
...
fn run_phase(phase: &str, use_kernel: bool, ticks: u64) {
    ...
    println!("[{}] cpu_sistema_durante_bench={:.1}% temp_final={:.2}C", phase, cpu_pct, cpu_temp());
}
...
fn main() {
    ...
    println!("=== SENTINEL BENCH SALTO-17 [{}] kernel={} ===", phase, use_kernel);
    
    // Calentar
    for _ in 0..10 { thread::sleep(Duration::from_millis(1)); }

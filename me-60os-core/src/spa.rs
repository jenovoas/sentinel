...
impl SPA {
    ...
    pub const fn two_pi() -> Self { Self::TWO_PI }
...
impl Add for SPA {
    ...
    fn add(self, other: Self) -> Self {
        let raw = self.to_raw().checked_add(other.to_raw()).expect("SPA overflow en add");
        Self::from_raw(raw)
    }
}
...
impl Sub for SPA {
    ...
    fn sub(self, other: Self) -> Self {
        let raw = self.to_raw().checked_sub(other.to_raw()).expect("SPA overflow en sub");
        Self::from_raw(raw)
    }
}
...
impl Mul<i64> for SPA {
    ...
    fn mul(self, scalar: i64) -> Self {
        let raw = self.to_raw().checked_mul(scalar).expect("SPA overflow en mul escalar");
        Self::from_raw(raw)
    }
}

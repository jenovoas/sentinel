...
impl PatternDetector {
            ...
            return Some(DetectedPattern {
                ...
                events: events
                    ...
                    .filter(|e| {
                        matches!(
                            e.event_type,
                            EventType::FailedLogin | EventType::SuccessfulLoginNewIP
                        )
                    })
                    .cloned()
                    .collect(),
    ...
    fn detect_ddos(&self, events: &[Event]) -> Option<DetectedPattern> {
        let network_spikes = events
            .iter()
            .filter(|e| e.event_type == EventType::CpuSpike)
            .count();
    ...
    fn detect_ransomware(&self, events: &[Event]) -> Option<DetectedPattern> {
        let unauth_io = events
            .iter()
            .any(|e| e.event_type == EventType::UnauthorizedAccess);
        if unauth_io {
            return Some(DetectedPattern {
    ...
    fn detect_privilege_escalation(&self, events: &[Event]) -> Option<DetectedPattern> {
        let priv_esc = events
            .iter()
            .any(|e| e.event_type == EventType::PrivilegeEscalation);
        if priv_esc {
            return Some(DetectedPattern {
...
mod tests {
    ...
    fn test_credential_stuffing_detected() {
        ...
        let events: Vec<Event> = (0..60)
            .map(|_| make_event(EventType::FailedLogin))
            .collect();
        let patterns = detector.detect(&events);
        assert!(patterns
            .iter()
            .any(|p| p.name.contains("Credential Stuffing")));
    }
